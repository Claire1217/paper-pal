...
npm run animation      # regenerates docs/images/flow-*.svg and confirm-*.svg (see below)
```
...
`docs/images/logo.svg` (with `logo-dark.svg` for dark backgrounds), the README lockup `wordmark.svg` / `wordmark-dark.svg` (mark plus the name as outlines), and a 32-unit version for the favicon and `.brand-mark` in
...
The animated pictures in the READMEs are separate: `npm run animation` runs
`scripts/readme-animation.mjs` and rewrites `docs/images/flow-*.svg` (select, comment, diff, accept)
and `confirm-*.svg` (confirm a read passage, compile the PDF). They are drawings of the app, not
recordings: one SVG per scene and theme, animated with CSS keyframes only, because that is what
